...
async createWithBarcode(dto: CreateProductDto): Promise<Product> {
  ...
  findAll() {
    return this.productModel.find().populate('brand').populate('supplier').populate('variants').populate('category');
  }

  findOneByBarcode(barcode: string) {
    return this.productModel.findOne({ barcode }).populate('brand').populate('supplier').populate('variants').populate('category');
  }


  findOne(id: string) {
    return this.productModel.findById(id).populate('brand').populate('supplier').populate('variants').populate('category');
  }
